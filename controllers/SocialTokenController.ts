...
const getToken = async (req: Request, res: Response): Promise<object> => {
  ...
  if (tokenType === "twitter_token") {
    try {
      const token = await SocialToken.select(["access_token", "access_token_secret"])
...
const storeToken = async (req: Request, res: Response) => {
  ...
  try {
    if (access_token) {
      ...
      const alreadySelectedSite = await CurrentSiteForToken.select(["current_id"])
        .where("token_type", "=", tokenType)
        .get();
      // if it exists remove it from the db
      if (alreadySelectedSite.length !== 0) {
        await client.query("DELETE FROM current_site_for_token WHERE current_id=$1", [
...
const setCurrentSiteForToken = async (req: Request, res: Response) => {
  const { websiteUrl, tokenType } = req.body;
  try {
    const checkIfAWebsiteIsAlreadySelected = await CurrentSiteForToken.select(["current_id"])
      .where("token_type", "=", tokenType)
      .get();
    if (checkIfAWebsiteIsAlreadySelected.length !== 0) {
      // update the already entered entry if it exists instead of creating a new one
      await pool.query("UPDATE current_site_for_token SET website_url=$1,token_type=$2 WHERE current_id=$3", [
        websiteUrl,
        tokenType,
        checkIfAWebsiteIsAlreadySelected[0].current_id,
      ]);
      return res.json({
        success: true,
    ...
    await CurrentSiteForToken.insert(["website_url", "token_type"], [websiteUrl, tokenType]);
    return res.json({
      success: true,
      message: "Site for token set",
    });
...
const getCurrentSelectedSiteUrl = async (req: Request, res: Response) => {
  ...
  try {
    const websiteUrl = await CurrentSiteForToken.select(["website_url"]).where("token_type", "=", tokenType).get();
...
const storeTemporaryToken = async (req: Request, res: Response): Promise<object> => {
...
const getTemporaryToken = async (req: Request, res: Response): Promise<object> => {
  ...
  const tokens = await TemporaryAccessTokens.select(["access_token", "access_token_secret"])
    .where("website_url", "=", websiteUrl)
    .get();
  // if no token found
  if (tokens.length === 0) {
 ...
 */
const deleteSocialIntegrationToken = async (req: Request, res: Response): Promise<object> => {
